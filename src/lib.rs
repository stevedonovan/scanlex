...
impl<'a> Scanner<'a> {
    ...
    pub fn get(&mut self) -> Token {
        ...
        let plusminus = if !self.no_float {
            self.either_plus_or_minus()
        } else {
            None
        };
        if self.is_digit() || plusminus.is_some() {
            ...
            if self.ch != '.' {
                // for 0. case - we already peeked ahead
    ...
    pub fn get_string(&mut self) -> Result<String, ScanError> {
        self.get()
            .to_string_result()
            .map_err(|e| self.update_lineno(e))
    }
...
mod tests {
    ...
    fn line_comments() {
        let text = "
            ...
            20
        ";
        let mut scan = Scanner::new(text).line_comment('#');
